...
if(isset($_POST["title"]) && isset($_POST["grade"]) && isset($_POST["subject"])){
    ...
    $note_title_rs = Database::search("SELECT * FROM `notes` WHERE `title` = '".$title."'");
    $note_title_num = $note_title_rs->num_rows;

    if(empty($title)){
        echo("Please insert the title.");
    }else if(strlen($title) > 150){
        echo("Title cannot exceed more than 150 characters.");
    }else if($note_title_num != 0){
        echo("This Note title has already been used.");
    }else if($grade == 0){
            ...
            if(isset($_FILES["file"])){
                ...
                if(in_array($file_extension,$allowed_extensions)){
                    ...
                    if($teacher_has_subject_rs->num_rows == 1){
                        ...
                        $file_name = "resources//notes//n_".uniqid()."_g".$grade."_".$subject_data["subject_name"].".pdf";
                        move_uploaded_file($file["tmp_name"],$file_name);
    
                        Database::iud("INSERT INTO `notes`(`title`,`path`,`teacher_has_subject_id`,`datetime`) 
